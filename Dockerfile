...
WORKDIR /app
CMD ["sh", "-c", "wp", "core", "install", "--title=$WP_TITLE", "--admin_user=$WP_ADMIN_USER", "--admin_password=$WP_ADMIN_PASSWORD", "--admin_email=$WP_ADMIN_EMAIL", "--allow-root", "&&", "wp", "plugin", "activate", "--all", "--allow-root"]
